...
func strings(for language: AppLanguage) -> GuideStrings {
    ...
    case .korean:
        return GuideStrings(
            ...
            tourTask2: "\u{201c}1장 - 지도에 없는 문\u{201d} 시나리오 — 대사 블록, 지침 블록, 분기(카일이 말리는 경우) 확인",
            tourTask3: "\u{201c}세계관 지도\u{201d} 마인드맵 — 노드 이동, 문서로 연결된 노드 더블클릭",
            tourCallout: "팁: 사이드바의 참조 패널에서 문서 간 백링크를 확인할 수 있어요.",
    ...
    case .japanese:
        return GuideStrings(
            ...
            tourTask2: "「第1章 - 地図にない扉」シナリオ — セリフブロック、指示ブロック、分岐（カイルが止める場合）を確認",
            tourTask3: "「世界観マップ」マインドマップ — ノードの移動、ドキュメントにリンクされたノードのダブルクリック",
            tourCallout: "ヒント: サイドバーの参照パネルでドキュメント間のバックリンクを確認できます。",
    ...
    case .english:
        return GuideStrings(
            ...
            tourTask2: "The \u{201c}Chapter 1 - The Door No Map Shows\u{201d} scenario — dialogue blocks, direction blocks, and the branch (\u{201c}If Kyle Talks Her Out Of It\u{201d})",
            tourTask3: "The \u{201c}World Map\u{201d} mind map — drag nodes, double-click the node linked to a document",
            tourCallout: "Tip: the References panel in the sidebar shows backlinks between documents.",
