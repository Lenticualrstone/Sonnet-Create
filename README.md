...
- 전체화면/윈도우 레이아웃 분기 — 헤더·픽셀 필드가 윈도우/전체화면에서 각각 다른 위치·크기로 렌더링(신호등 유무에 대응)
- 사이드바 재편 — 파일 아카이브 풀폭 버튼, 프로필 팝업(사진/소개 + 프로필·설정·가려진 항목·최근 지워진 항목), 프로필 사진/소개 편집(설정 > 일반)
- 모션 패스 — LiftButtonStyle(카드 호버 리프트+눌림, DesignSystem 공용), 저장 배지 펄스, 사이드바 탭 슬라이딩 하이라이트, AI 채팅 메시지 도착 트랜지션
